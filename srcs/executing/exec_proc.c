...
/*   Updated: 2022/08/17 11:30:32 by nelidris         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cmd_line *cmd_line, int pipeline)
{
	...
	if (!run_builtin(cmd_line, pipeline, NULL))
...
static pid_t	run_command(t_cmd_line **cmd, t_cmd_line *cmd_line,
						int pipeline, int *exit_code)
{
	pid_t	pid;
	...
	if (!pipeline && !run_builtin(cmd_line, pipeline, exit_code))
		return (-1);
	pid = fork();
	if (pid < 0)
		return (pid);
...
int	execute_cmd_line(t_cmd_line **cmd_line)
{
	...
	while (cmd_line[index])
		pid = run_command(cmd_line, cmd_line[index++], more, &exit_code);
	ret_exit = 0;
	more = 0;
	while (more != -1)
	{
